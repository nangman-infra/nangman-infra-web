...
export function getLlmsText(): string {
  return [
    ...
    "- Blog detail pages on this domain are summary/index pages for member articles. They should be used for discovery, and the linked external source URL should be preferred when exact wording from the original article matters.",
...
export function getLlmsFullText(): string {
  return [
    ...
    "- The site mixes primary-source organization pages with curated blog summary pages that point to original member articles.",
    "",
    ...
    "- Curated technical blog index and summary pages: /blog and /blog/*",
    "",
    "## Primary Pages",
    ...PRIMARY_PAGES.map(
      (page) => `- ${page.name}: ${page.url} | ${page.description}`,
    ...
    "- For blog content, nangman.cloud/blog/* pages are internal summary pages. Follow the linked external source URL for the original full article.",
    ...
    "- Use caution with blog summaries on nangman.cloud when exact wording matters; cite the external source article instead.",
    "",
  ].join("\n");
}
